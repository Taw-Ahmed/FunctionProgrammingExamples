...
namespace ConsoleApp19
{
    ...
    public void Main()
        ...
        // each order could be qualified to multiple rules
        // each rule has its own critieria 
        // if the prder qualified to multiple rules which means multiple dicounts we get the lowest 3 rules and get thier average as final discount 
       public Order GetOrderwithDiscount(Order R, List<(Func<Order, bool> QualifyingCondition, Func<Order, decimal> GetDiscount)> Rules)
        {
        	var discount = Rules.Where((a) => a.QualifyingCondition(R))
                                               .Select((b) => b.GetDiscount(R))
                                               .OrderBy((x) => x)
                                               .Take(3).Average();
